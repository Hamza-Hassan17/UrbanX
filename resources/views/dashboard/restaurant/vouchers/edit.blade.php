...
@section('title', __('Edit Restaurant Voucher'))
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.restaurant-vouchers.update', $voucher->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row p-5">
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <select id="discount_type" name="discount_type" class="select2 form-select @error('discount_type') is-invalid @enderror" required>
                                ...
                                <option value="fixed" {{ old('discount_type', $voucher->discount_type) == 'fixed' ? 'selected' : '' }}>{{__('Fixed')}}</option>
                                <option value="percentage" {{ old('discount_type', $voucher->discount_type) == 'percentage' ? 'selected' : '' }}>{{__('Percentage')}}</option>
                            </select>
                            @error('discount_type')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('expires_at') is-invalid @enderror" type="date" id="expires_at"
                                ...
                                value="{{ old('expires_at', \Carbon\Carbon::parse($voucher->expires_at)->format('Y-m-d')) }}" />
                            @error('expires_at')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                placeholder="{{ __('Enter description') }}" cols="30" rows="10">{{ old('description', $voucher->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Edit Voucher') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
