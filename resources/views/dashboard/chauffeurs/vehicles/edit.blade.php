...
@section('title', __('Update Chauffeurs Vehicle'))
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.chauffeur-vehicles.update', $vehicle->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row p-5">
                                ...
                                @if (isset($carBrands) && count($carBrands) > 0)
                                    @foreach ($carBrands as $brand)
                                        <option value="{{ $brand->id }}"
                                            {{ $brand->id == old('car_brand_id', $vehicle->car_brand_id) ? 'selected' : '' }}>{{ $brand->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="mb-4 col-md-4">
                            <label for="car_model" class="form-label">{{ __('Car Model') }}</label><span
                                class="text-danger">*</span>
                            <input class="form-control @error('car_model') is-invalid @enderror" type="text" id="car_model" name="car_model" value="{{ old('car_model', $vehicle->car_model) }}" required
                        ...
                        <div class="mb-4 col-md-4">
                            ...
                            <input class="form-control @error('car_fuel_type') is-invalid @enderror" type="text"
                                id="car_fuel_type" name="car_fuel_type" value="{{ old('car_fuel_type', $vehicle->car_fuel_type) }}" required
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('price_per_hour') is-invalid @enderror" type="number" min="0" step="0.01"
                                id="price_per_hour" name="price_per_hour" value="{{ old('price_per_hour', $vehicle->price_per_hour) }}" required
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('price_per_day') is-invalid @enderror" type="number" min="0" step="0.01"
                                id="price_per_day" name="price_per_day" value="{{ old('price_per_day', $vehicle->price_per_day) }}" required
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('price_per_week') is-invalid @enderror" type="number" min="0" step="0.01"
                                id="price_per_week" name="price_per_week" value="{{ old('price_per_week', $vehicle->price_per_week) }}" required
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('price_per_month') is-invalid @enderror" type="number" min="0" step="0.01"
                                id="price_per_month" name="price_per_month" value="{{ old('price_per_month', $vehicle->price_per_month) }}" required
                        ...
                        <div class="mb-4 col-md-4">
                            ...
                            <select id="transmission" name="transmission" class="select2 form-select @error('transmission') is-invalid @enderror" required>
                                ...
                                <option value="semi-automatic"{{ old('transmission', $vehicle->transmission) == 'semi-automatic' ? 'selected' : '' }}>{{__('Semi Automatic')}}</option>
                                <option value="cvt"{{ old('transmission', $vehicle->transmission) == 'cvt' ? 'selected' : '' }}>{{__('CVT')}}</option>
                            </select>
                            @error('transmission')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            @if ($vehicle->main_image)
                                <div class="mt-3">
                                    <img src="{{ asset($vehicle->main_image) }}" alt="Main Image" width="150" height="100">
                                </div>
                            @endif
                        </div>
                        <div class="mb-4 col-md-12">
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                placeholder="{{ __('Enter description') }}" cols="30" rows="10">{{ old('description', $vehicle->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="col-md-6 mb-4">
                            <label class="switch switch-square">
                                ...
                                <input type="checkbox" class="switch-input @error('is_featured') is-invalid @enderror" id="is_featured"
                                    name="is_featured" {{ old('is_featured', $vehicle->is_featured) == '1' ? 'checked' : '' }}/>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Edit Vehicle') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
